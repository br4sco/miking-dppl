include "../lib.mc"
include "../lotka-model.mc"

let join = lam xs : [[FloatA]]. foldl (lam y : [FloatA]. concat y) [] xs

-- Specialize solveode
let solve = lam f : [FloatA] -> [FloatA]. lam y0 : [FloatA]. lam dxs : [Float].
  let s = lam f : [FloatA] -> [FloatA]. lam y0 : [FloatA]. lam x : Float.
    solveode (Default { add = adds, smul = smuls, stepSize = 1e-3 })
      (lam x : FloatA. f) y0 x
  in
  reverse
    (foldl (lam ys : [[FloatA]]. lam dx : Float. cons (s f (head ys) dx) ys)
       [y0] dxs)

let sns = lam theta : FloatA. lam n : Int. lam dx : Float.
  -- Times-steps
  let dxs = create n (lam i : Int. dx) in

  -- Jacbian-vector product.
  let jvp = lam f : [FloatA] -> [FloatA]. lam x : [FloatA]. diff f x in

  -- Function that takes a parametrized ODE model as input and
  -- outputs a ODE model augmented with sensitivity equations.
  let augmentSens = lam f : [FloatA] -> [FloatA] -> [FloatA]. lam ny : Int.
    lam theta : [FloatA]. lam y : [FloatA].
      if neqi (muli (addi (length theta) 1) ny) (length y) then
        error
          (concat "augmentSens: incorrect number of states: "
             (float2string (int2float (length y))))
      else
      -- Group the dependent variables:
      --  y = [y_1, .., y_n,
      --       (dydθ_1)_1, .., (dydθ_1)_ny,
      --       ..,
      --       (dydθ_nθ)_1, .., (dydθ_nθ)_ny]
      -- into:
      --   [[y_1, .., y_ny],
      --    [(dydθ_1)_1, .., (dydθ_1)_ny],
      --    ..,
      --    [(dydθ_nθ)_1, .., (dydθ_nθ)_ny]]
      let ys =
        foldl
          (lam acc : [[FloatA]]. lam y : FloatA.
            switch acc
            case [] then error "impossible"
            case yss ++ [ys] then
              if lti (length ys) ny then snoc yss (snoc ys y)
              else snoc acc [y]
            end)
          [[]]
          y
      in
      match ys with [y] ++ sns in
      concat
        -- original ODE model: f(x), in x' = f(x).
        (f theta y)
        (join
           (mapi
              (lam i : Int. lam sns : [FloatA].
                -- sensitivity model: (dx/dθ_i)' = (df/dx)(dx/dθ_i) + df/dθ_i
                adds
                  (jvp (f theta) y sns)
                  (jvp (lam theta : [FloatA]. f theta y)
                     theta (onehots (length theta) i)))
              sns))
  in

  let lotkaVolterra = lam theta : FloatA. lotkaVolterra (theta, 1., 1., 3.) in

  -- We parametrize our ODE model on the prey per-capita growth rate.
  let ode1 = lam theta : [FloatA]. lam y : [FloatA].
    match theta with [theta] in
    match lotkaVolterra theta (get y 0, get y 1) with (f1, f2) in
    [f1, f2]
  in

  -- Initial values
  let y0 = [1., 1.] in

  -- The number of dependent variables
  let ny = length y0 in

  -- We also create a second ODE model that includes the sensitivities directly.
  let ode2 = augmentSens ode1 ny in

  -- Trace the sensitivites w.r.t. the parameter by differentiating the solution
  -- trace.
  let d = lam f : [FloatA] -> [[FloatA]]. lam x : [FloatA]. diff f x in
  let sns1 = d (lam theta : [FloatA]. solve (ode1 theta) y0 dxs) [theta] in

  -- We also compute the forward continous sensitvities.
  let sns2 =
    map
      (lam y : [FloatA].
        match y with [_, _, s1, s2] then [s1, s2] else error "impossible")
      (solve (ode2 [theta]) (concat y0 [0., 0.]) dxs)
  in

  -- Return the distribution of the two traces
  (sns1 [1.], sns2)

mexpr

-- Print the sensitivites for the two methods for a number of random parameters.
-- plot output with `dppl-plot-process` script. I.e., from the repository root
-- ./build/cppl --effect-frontend coreppl/test/coreppl-to-mexpr/dppl/examples/ode-sensitivites.mc && ./out | ./scripts/dppl-plot-process && rm ./out
let printDist = false in

-- Number of time-steps
let n = 100 in
-- Size time-step
let dx = 0.1 in

-- Compare the sensitivites from the two methods
match sns 1.1 n dx with (sns1, sns2) in
utest join sns1 with join sns2 using eqs in

(if printDist then
  let model = lam t : ().
    -- Uniformly draw the model parameter
    let theta = assume (Uniform 1.1 1.5) in

    -- Output on a format suitable for the `dppl-plot-process` script
    match sns theta n dx with (sns1, sns2) in
    let f = mapi (lam i : Int. lam y : [FloatA]. (mulf (int2float i) dx, y)) in
    [f sns1, f sns2]
  in

  -- Draw a number of samples and print them
  let sns = infer (Importance { particles = 100 }) model in
  printODETracesDist sns
 else ());

()
