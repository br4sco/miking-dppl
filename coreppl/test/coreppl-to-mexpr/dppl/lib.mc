...
let absf = lam x : Float. if ltf x 0. then negf x else x

-- `eqfApprox e a b` is true if |a - b| < e.
let eqfApprox = lam e : Float. lam a : Float. lam b : Float. ltf (absf (subf a b)) e
...
-- Vector equality
let eqs = lam a : [Float]. lam b : [Float].
  foldl and true (map (lam t : (Float, Float). eqfApprox 0.05 t.0 t.1) (zip a b))
...
let printODETracesDist = lam dist : Dist [[(Float, [Float])]].
  match distEmpiricalSamples dist with (samples, weights) in
  iteri
    (lam i : Int. lam s : [[(FloatN, [FloatN])]].
      ppODETraces s;
