...
lang DTCTypeError = Ast + DTCEnv + DTCFloatTypeAst + PrettyPrint
  ...
  sem _floatapstr =| () -> join [
    type2str (tyfloatc_ (A ())), "|", type2str (tyfloatc_ (P ()))
  ]
  ...
  | DTCSolveODEModelError (Some (info, ty)) ->
    let str =  _floatapstr () in
    (info,
     _typeErrorToMsg2
       ["Function type (", str ,") -> T -> T,",
        "where T isomorfic to (", str, ",...,", str, ")"]
       [type2str ty])
      ...
      (lam method. lam model. lam init. lam endTime.
        ...
        then
          if allb [
            ...
            in
            let modelTy = TyArrowE { arr1 with to = tyarrowe_ ty ty (Det ())} in
